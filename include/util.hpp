...
bool call_generals(GameState &gamestate, int player, std::pair<int, int> location)
{
  ...
  else if (gamestate.board[location.first][location.second].generals != nullptr)
  {
    return false;
  }
...
* 返回值：新的位置坐标。 */
std::pair<int, int> calculate_new_pos(std::pair<int, int> location,
                                      Direction direction)
{
...
bool army_move(const std::pair<int, int> location, GameState &gamestate,
               int player, Direction direction, int num)
{
  ...
  if (num <= 0)
    return false; // 移动数目非法
  if (num >= gamestate.board[x][y].army - 1)
  { // 超过最多移动兵力
  ...
  { // 超级武器效果
    if (sw.position == location && sw.rest &&
        sw.type == WeaponType::TRANSMISSION)
    { // 超时空传送眩晕
      return false;
    }
    if (std::abs(sw.position.first - x) <= 1 &&
        std::abs(sw.position.second - y) <= 1 && sw.rest &&
  ...
  if (newX < 0)
    return false; // 越界
  if (gamestate.board[newX][newY].type == CellType::SWAMP &&
      gamestate.tech_level[player][1] == 0)
  { // 不能经过沼泽
    return false;
  }
  ...
  if (gamestate.board[newX][newY].player == player)
  { // 目的地格子己方所有
  ...
  { // 攻击敌方或无主格子
    ...
    { // 攻下
      ...
      gamestate.board[x][y].army -= num;
      if (gamestate.board[newX][newY].generals != nullptr)
      { // 将军易主
                                            ...
                                            const std::pair<int, int> destination)
{
  ...
  if (gamestate.board[x][y].player != player ||
      gamestate.board[x][y].generals == nullptr)
  {
  ...
  while (op < cl)
  {
    ...
    for (const auto &direction : directions)
    {
      ...
      if (gamestate.board[newP][newQ].type == CellType::SWAMP &&
          gamestate.tech_level[player][1] == 0)
      {
        continue; // 无法经过沼泽
      }
      if (gamestate.board[newP][newQ].player != player ||
          gamestate.board[newP][newQ].generals != nullptr)
      {
        continue; // 目的地格子非法
      }
...
* 返回值：如果移动成功，返回 `true`；否则返回 `false`。 */
bool general_move(
    const std::pair<int, int> location, GameState &gamestate, int player,
    const std::pair<int, int> destination)
{ // 将军移动（参数须保证合法）
...
bool check_rush_param(int player, std::pair<int, int> destination, std::pair<int, int> location, GameState &gamestate)
{
  ...
  // 检查参数合理性
  if (gamestate.board[x][y].generals == nullptr)
    return false; // 如果当前位置没有将军，返回失败
  if (gamestate.board[x_new][y_new].generals != nullptr)
    return false; // 如果目标位置有将军，返回失败
  if (gamestate.board[x_new][y_new].army < 2)
    return false; // 如果目标位置的军队数量小于2，返回失败
...
bool skill_activate(int player, std::pair<int, int> location, std::pair<int, int> destination, GameState &gamestate, SkillType skillType)
{
  ...
  if (general == nullptr)
  {
    return false; // 如果指定位置上没有将领，则返回false
  }
...
bool handle_bomb_cell(GameState &gamestate, int x, int y)
{
  ...
  // 如果单元格中有主将军，将军队数量减半
  if (dynamic_cast<MainGenerals *>(cell.generals))
  {
...
bool production_up(std::pair<int, int> location, GameState &gamestate, int player)
{
  if (gamestate.board[location.first][location.second].generals != nullptr)
  {
