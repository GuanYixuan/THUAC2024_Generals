...
bool GameController::execute_single_command(int player, const Operation &op)
{
    ...
    case OperationType::UPDATE_GENERALS:
    {
        ...
        case 1:
            // 提升生产力
            return production_up(game_state.generals[id].position, game_state, player);
        case 2:
            // 提升防御力
            return defence_up(game_state.generals[id].position, game_state, player);
        case 3:
            // 提升移动力
            return movement_up(game_state.generals[id].position, game_state, player);
        }
    ...
    case OperationType::USE_GENERAL_SKILLS:
    {
        ...
        if (params[1] == 1 || params[1] == 2)
            ...
            return skill_activate(player, game_state.generals[id].position, {-1, -1}, game_state, static_cast<SkillType>(params[1] - 1));
    }
