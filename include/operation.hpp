...
public:
    ...
    Operation(OperationType opcode = OperationType::DEFAULT_OP, const std::vector<int>& operand = {}) : opcode(opcode), operand(operand) {}
...
Operation generals_skill_op(int generals_id, SkillType type, std::pair<int,int> position) {
    return Operation(OperationType::USE_GENERAL_SKILLS, {generals_id, static_cast<int>(type) + 1, position.first, position.second});
}

/* 创建升级科技的操作对象 */
Operation update_tech_op(TechType type) {
...
Operation use_superweapon_op(WeaponType type, std::pair<int,int> destination, std::pair<int,int> origin={-1,-1}) {
    return Operation(OperationType::USE_SUPERWEAPON, {static_cast<int>(type) + 1, destination.first, destination.second, origin.first, origin.second});
}
